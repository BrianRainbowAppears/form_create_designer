    ...
    <el-container class="_fc-designer" @dragenter="handleDragenter" @dragleave="handleDragleave" @drop="handleDrop">
        ...
        t('com.' + element.name + '.name') || element.label
                        ...
                        <el-main v-if="activeMenuTab === 'tree'">
                            <el-tree ref="treeRef" :data="treeInfo" default-expand-all :expand-on-click-node="false"
                                ...
                                <template #default="{ node, data }">
                                    <div class="_fc-tree-node" :class="{ active: activeRule === data.rule }">
                                        <div class="_fc-tree-label">
                                            ...
                                            <span>{{
        (data.rule.title || '').trim() || (data.rule.props &&
            data.rule.props.label) || t('com.' + (data.rule._menu &&
                data.rule._menu.name) + '.name') || data.rule.type
    }}</span>
                                        </div>
        ...
        t('props.save')
                                ...
                                <el-button type="primary" plain size="small" @click="openPreview"><i
                                        class="fc-icon icon-preview"></i> {{
        t('props.preview')
    }}
                                </el-button>
                    ...
                    <el-container style="height: 100%;">
                        ...
                        <el-main class="_fc-r-tab-form" v-show="activeTab === 'form'"
                            ...
                            <DragForm :rule="form.rule" :option="form.option" :modelValue="form.value"
                                @change="formOptChange" v-model:api="form.api"></DragForm>
                        </el-main>
                        <el-main class="_fc-r-tab-props" v-show="activeTab === 'props'"
                            ...
                            <template
                                ...
                                <el-tag type="success" effect="plain" disable-transitions>
                                    ...
                                    <template v-else>
                                        {{
        t('com.' + (customForm.config.name) + '.name') || customForm.config.label ||
        customForm.config.name
                            ...
                            <template v-if="activeRule">
                                ...
                                <DragForm v-if="validateForm.isShow" v-model:api="validateForm.api"
                                    ...
                                    :modelValue="validateForm.value" @change="validateChange" :key="activeRule._fc_id">
                                </DragForm>
                            </template>
                        </el-main>
                    </el-container>
...
export default defineComponent({
    ...
    setup (props) {
        ...
        // Watch for changes in data.activeTab
        watch(() => data.activeTab, async function (newVal, oldVal) {
            ...
            //     data.someSpecificTabData = result;
            // } catch (error) {
        ...
        const methods = {
            ...
            async handleSave () {
                ...
                try {
                    const response = await fetch('/api/openapi/tenant/setting/list', {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                            'data-application': '11fe87e6-6191-4995-b924-0cea60202275',
                            'data-authorization': 'Basic ODQ3ZGMwMjQtNmNiYi00ZDJjLTlmNzktNDk2MmIyMzdkZmUyOmE4OTc5MGMzLWJkOTctNDc1Ni04ODgzLTM1ZGY2MGE1ZDE1NQ==',
                            'hdc-encryption-enabled': 'false',
    ...
    created () {
        console.log('data.menuList', this.menuList);
